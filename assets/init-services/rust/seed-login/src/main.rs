...
async fn main() -> Result<(), mongodb::error::Error> {
    ...
    collection.create_index(mongodb::IndexModel::builder().keys(mongodb::bson::doc! {"username": 1}).build()).await?;
    collection.create_index(mongodb::IndexModel::builder().keys(mongodb::bson::doc! {"email": 1}).build()).await?;
